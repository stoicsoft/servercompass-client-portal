...
import { validateBrowserRequest } from './security.js';
...
function proxy(request, response) {
  ...
  const forwardedChain = String(request.headers['x-forwarded-for'] ?? request.socket.remoteAddress ?? '').split(',');
  const forwardedFor = forwardedChain[forwardedChain.length - 1].trim();
  const headers = {
